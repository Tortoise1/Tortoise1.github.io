...
// 复制地址：优先 clipboard API，X5 内核失败时退回 execCommand
...
function legacyCopy(text) {
  ...
  showToast(copied ? '已复制地址' : '请长按地址手动复制');
}

const WEDDING_DATE = Date.UTC(2026, 9, 25);
const SHANGHAI_TIME_ZONE = 'Asia/Shanghai';
const SPRITE_SHEET_URL = './sprite.jpg';
...
musicHeart.addEventListener('click', async (event) => {
  ...
  try {
    ...
    // 容器可能拦截自动播放：即使音乐没响，也必须放行页面，避免用户卡在首屏
    unlockBlessing();
    activateSpriteParade();
    showToast('音乐未开启，可点右上角音符再试一次');
  } finally {
    musicHeart.classList.remove('is-activating');
    window.setTimeout(hideMusicHeart, 560);
  }
});

// 兜底：点击后 1.2 秒仍未解锁，就强制解锁，保证任何环境下都能往下翻
musicHeart.addEventListener('click', () => {
  window.setTimeout(() => {
    if (document.body.classList.contains('blessing-locked')) unlockBlessing();
  }, 1200);
});
...
function updateWeddingCountdown() {
  ...
  // Chrome 61 无 Object.fromEntries（ES2019），手动聚合
  const dateParts = {};
  today.forEach(function (part) { dateParts[part.type] = part.value; });
